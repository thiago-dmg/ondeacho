...
class _ListingDetailsPageState extends ConsumerState<ListingDetailsPage> {
  ...
  Future<void> _openExternalUri(
    ...
    if (!opened) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            fallbackMessage ?? "Não foi possível abrir o aplicativo.",
          ),
        ),
      );
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
          icon: const LIcon(LucideIcons.arrowLeft),
          tooltip: "Voltar",
          onPressed: () {
            if (context.canPop()) {
              context.pop();
                    ...
                    onPressed: () async {
                      ...
                      try {
                        ...
                        if (context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text("Erro ao favoritar: $error"),
                            ),
                          );
                        }
                      }
                    ...
                    PopupMenuButton<String>(
                      ...
                      itemBuilder: (context) => [
                        if (isLoggedIn && clinic.viewerIsOwner)
                          const PopupMenuItem(
                            value: "edit",
                            child: Text("Editar dados da clínica"),
                          ),
                        if (!clinic.isClaimed)
                          const PopupMenuItem(
                            value: "claim",
      ...
      body: clinicAsync.when(
                  ...
                  child: Column(
                    ...
                    children: [
                              ...
                              child: FilledButton.icon(
                                onPressed: () async {
                                  await _openExternalUri(
                                    ...
                                    Uri(
                                      scheme: "tel",
                                      path: _digitsOnly(normalizedPhone),
                                    ),
                                    fallbackMessage:
                              ...
                              child: FilledButton.icon(
                                onPressed: () async {
                                  await _openExternalUri(
                                    context,
                                    Uri.parse(
                                      "https://wa.me/${_digitsOnly(normalizedWhatsapp)}",
                                    ),
                                    fallbackMessage:
                                    ...
                                    OutlinedButton.icon(
                                      onPressed: () async {
                                        await _openExternalUri(
                                          ...
                                          Uri.parse(fb),
                                          fallbackMessage:
                                              "Não foi possível abrir o Facebook.",
                                        );
                                      },
                                      icon: const LIcon(
                      ...
                      const AppSectionHeader(
                        title: "Profissionais nesta clínica",
                      ),
                      const SizedBox(height: 8),
                      ...
                      if (!clinic.isClaimed)
                        Center(
                          child: TextButton(
                            ...
                            child: const Text(
                              "Sou responsável por esta clínica",
                            ),
                          ),
                        )
                      else
        ...
        error: (error, stack) =>
            Center(child: Text("Erro ao carregar detalhes: $error")),
      ),
    );
  }
}
